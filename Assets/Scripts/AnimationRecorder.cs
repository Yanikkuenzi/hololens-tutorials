    ...
    void LateUpdate()
    {
        ...
        try
        {
            ...
            if (nPoints > 0)
            {
                ...
                {
                    ((Vector3[])points[idx])[i] = new Vector3(pointCloud[3 * i], pointCloud[3 * i + 1], pointCloud[3 * i + 2]);
                    // TODO: capture color
            ...
            using (StreamWriter writer = new StreamWriter(directory + "/" + string.Format("{0:D6}.ply", i)))
            {
                ...
                for(int j = 0; j < curr_points.Length; j++)
                {
                    writer.WriteLine(string.Format("{0} {1} {2} {3} {4} {5}",
                        ...
                        curr_colors[j].r, curr_colors[j].g, curr_colors[j].b));
                }
            }
